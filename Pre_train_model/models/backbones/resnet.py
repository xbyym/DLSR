...
from Pre_train_model.models.initializer import initialize_from_cfg
...
def build_resnet(model_name, pretrained, layers, pretrained_model="", **kwargs):
    ...
    # pretrained=True
    # pretrained_model = "/data1/yyang/yyang/dlsr/state_dicts/resnet34.pt"
    if pretrained:
        if pretrained_model and os.path.exists(pretrained_model):
            state_dict = torch.load(pretrained_model)
            logger.info(f"Loading pretrained weights from {pretrained_model}")
        else:
            logger.info(
